...
def populate(request):
	...
	try:
		...
		cur = conn.cursor()
		cur.execute('DELETE FROM ex04_movies')
		for m in data:
			values = [m['episode_nb'], m['title'], m['director'], m['producer'], m['release_date']]
			insert = sql.SQL('INSERT INTO ex04_movies (episode_nb, title, director, producer, release_date) VALUES ({})').format(sql.SQL(',').join(map(sql.Literal, values)))
			cur.execute(insert)
		conn.commit()
		conn.close()
	except psycopg2.Error as e:
...
def remove(request):
	try:
		...
		if movies_lst:
			form = RemoveForm()
			return render(request, 'ex04/form.html', {
					'title' : 'ex04',
					'movies_lst': movies_lst,
					'form': form,
			})
...
def post(request):
	if request.method == 'POST':
		try:
			...
			cur = conn.cursor()
			titles = request.POST.get('_')
			cur.execute("DELETE FROM ex04_movies WHERE title = '%s'"% (titles))
			conn.commit()
			conn.close()
		except psycopg2.Error as e:
			return HttpResponse(str(e.pgerror).replace('\n', '<br />'))
